...
class ChooseSubscription(
    ...
    fun checkSubscriptionStatus(
        ...
        billingClient.queryPurchasesAsync(
            ...
            Log.d(TAG, "checkSubscriptionStatus:  result $result")
            Log.d(TAG, "checkSubscriptionStatus: map ${purchases.map { it.products }}")
            purchases.forEach {
                Log.d(TAG, "checkSubscriptionStatus: purchase: ${it.packageName}")
                it.products.forEach{
                    Log.d(TAG, "checkSubscriptionStatus:products $it")
                }
            }
            when (result.responseCode) {
                BillingResponseCode.OK -> {
        ...
        billingClient.queryProductDetailsAsync(queryProductDetailsParams) { billingResult, productDetailsList ->
            if (billingResult.responseCode == BillingResponseCode.OK) {
                ...
                val productDetails = productDetailsList.firstOrNull { productDetails ->
                    Log.d(TAG, "querySubscriptionPlans: " +
                            "\n${productDetails}" +
                            "\n${productDetails.name}" +
